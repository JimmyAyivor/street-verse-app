...
import UserTableMenu from "../Menus/UserTableMenu.js";
import CardSettings from '../Cards/CardSettings';

export default function Modal({ user, handleSubmit, handleTextChange, handleZipChange , handleDelete}) {
  const [showUserViewModal, setUserViewModal] = useState(false);
  ...
  const viewModal = (state) => {
    state === "open" ? setUserViewModal(true) : setUserViewModal(false)
  }
  const editModal = (state) => {
    state === "open" ? setUserEditModal(true) : setUserEditModal(false)
  }
  const deleteModal = (state) => {
    state === "open" ? setUserDeleteModal(true) : setUserDeleteModal(false)
  }
  ...
  return (
    <>
    <UserTableMenu viewModal={viewModal} editModal={editModal}  deleteModal={ deleteModal} />
      {showUserViewModal ? (
        <>
          <div
          ...
          >
            <div className="relative w-auto my-6 mx-auto max-w-3xl">
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="relative p-6 flex-auto">
                <CardSettings user={user} handleSubmit={handleSubmit} handleTextChange={handleTextChange} handleZipChange={handleZipChange} />
                </div>
                {/*footer*/}
                <div className="flex items-center justify-end p-6 border-t border-solid border-slate-200 rounded-b">
                  <button
